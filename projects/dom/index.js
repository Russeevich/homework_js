...
function findAllPSiblings(where) {
    ...
    for (let i = 0; i < where.children.length - 1; i++) {
        if (where.children[i].nextElementSibling.tagName === 'P') arr.push(where.children[i]);
    }
...
function deleteTextNodesRecursive(where) {
    where.nodeType === 3 ? where.remove() : [...where.childNodes].forEach(deleteTextNodesRecursive);
}
...
function collectDOMStat(root) {
    ...
    function getChild(items) {
        for (const item of items.childNodes) {
            if (item.hasChildNodes()) {
                getChild(item);
            }
            if (item.tagName)
                temp.tags[item.tagName] = temp.tags[item.tagName] ? temp.tags[item.tagName] + 1 : 1;

            if (item.classList)
                item.classList.forEach(classObj => {
                    temp.classes[classObj] = temp.classes[classObj] ? temp.classes[classObj] + 1 : 1;
                })
